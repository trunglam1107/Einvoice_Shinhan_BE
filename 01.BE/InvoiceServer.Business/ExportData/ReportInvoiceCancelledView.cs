...
{
    public class ReportInvoiceCancelledView : IXssf
    {
        ...
        public ReportInvoiceCancelledView(InvoiceExport dataReports, PrintConfig config, string language)
        {
            ...
            if (language == "en")
            {
                choooseFile = "report-invoice-cancelled-en.xlsx";
                this.lang = "en";
            }
            else
            {
                choooseFile = "report-invoice-cancelled-vn.xlsx";
                this.lang = "vi";
            }
        ...
        private void SetDataItemInvoiceToCells(InvoiceMaster item, int order, int rowIndex)
        {
            ...
            this.spireProcess.Worksheet.Range[string.Format("G{0}", rowIndex)].Text = this.lang == "en" ? "Cancelled" : "Đã hủy";
