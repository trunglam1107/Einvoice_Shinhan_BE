...
namespace InvoiceServer.Business.ExportXml
{
    public class InvoiceXml
    {
        ...
        private readonly ParseXML xml;
        private List<STATISTICAL> invoiceStatisticals { get; set; }
        private List<INVOICEGIFT> invoiceGift { get; set; }
        public InvoiceXml(InvoiceExportXmlModel invoiceExportXmlModel)
        {
            this.Config = invoiceExportXmlModel.PrintConfig;
            this.invoice = invoiceExportXmlModel.InvoicePrintInfo;
            this.systemSetting = invoiceExportXmlModel.SystemSettingInfo;
            //this.invoiceStatisticals = invoiceExportXmlModel.Statisticals.ToList();
            //this.invoiceGift = invoiceExportXmlModel.Invoicegifts.ToList();
            xml = new ParseXML(invoiceExportXmlModel);
        }
        ...
        private string CreateBills()
        {
            StringBuilder itemBill = new StringBuilder();
            if (this.invoiceStatisticals.Count == 0)
            {
                return itemBill.ToString();
            }
            foreach (var item in this.invoiceStatisticals)
            {
                itemBill.Append("<Bill>");
                itemBill.AppendFormat("<tradDate>{0}</tradDate>", item.TRADDATE.Value.ToString("dd/MM/yyyy").EscapeXMLValue());
                itemBill.AppendFormat("<currency>{0}</currency>", invoice.CurrencyCode.EscapeXMLValue());
                itemBill.AppendFormat("<amount>{0}</amount>", item.AMOUNT);
                itemBill.AppendFormat("<taxAmount>{0}</taxAmount>", item.AMOUNTTAX);
                itemBill.AppendFormat("<exchangeRate>{0}</exchangeRate>", item.EXCHANGERATE);
                itemBill.AppendFormat("<description>{0}</description>", item.DESCRIPTION.EscapeXMLValue());
                itemBill.Append("</Bill>");
            }
            return itemBill.ToString();
        }
        private string CreateGifts()
        {
            StringBuilder itemGift = new StringBuilder();
            if (this.invoiceGift.Count == 0)
            {
                return itemGift.ToString();
            }
            foreach (var item in this.invoiceGift)
            {
                itemGift.Append("<Gift>");
                itemGift.AppendFormat("<tradDate>{0}</tradDate>", item.CREATEDATE.Value.ToString("dd/MM/yyyy"));
                itemGift.AppendFormat("<currency>{0}</currency>", item.CURRENCY.EscapeXMLValue());
                itemGift.AppendFormat("<PreTaxAmount>{0}</PreTaxAmount>", item.PRETAXAMOUNT);
                itemGift.AppendFormat("<taxAmount>{0}</taxAmount>", item.AMOUNTTAX);
                itemGift.AppendFormat("<giftName>{0}</giftName>", item.GIFTNAME.EscapeXMLValue());
                itemGift.AppendFormat("<refNo>{0}</refNo>", item.REFNO.EscapeXMLValue());
                itemGift.Append("</Gift>");
            }
            return itemGift.ToString();
        }
