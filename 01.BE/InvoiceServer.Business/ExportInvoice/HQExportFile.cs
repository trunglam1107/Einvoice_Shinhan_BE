...
{
    public class HQExportFile : IXssf
    {
        private readonly InvoicePrintInfo invoiceInfo;
        ...
        public HQExportFile(InvoiceExportModel invoiceExportModel)
        {
            this.invoiceInfo = invoiceExportModel.InvoicePrintInfo;
            this.config = invoiceExportModel.PrintConfig;
            //if (invoiceExportModel.InvoiceStatisticals != null)
            //{
            //    this.invoiceStatisticals = invoiceExportModel.InvoiceStatisticals.ToList();
            //}
            //if (invoiceExportModel.InvoiceGift != null)
            //{
            //    this.invoiceGift = invoiceExportModel.InvoiceGift.ToList();
            //}

            //this.invoiceStatisticals = invoiceExportModel.InvoiceStatisticals;
            //this.invoiceGift = invoiceExportModel.InvoiceGift;
            ...
            this.SystemSetting = invoiceExportModel.SystemSetting != null ? invoiceExportModel.SystemSetting : this.invoiceInfo.CurrencyCode != "VND" ? invoiceExportModel.SystemSetting : new SystemSettingInfo();
        ...
        public HQExportFile(DeclarationExportModel exportModel)
        {
        ...
        public string ExportFileJob()
        {
            return ExportInvoiceJob();
        }
        private StringBuilder ReplaceHeader(string fullPathFileTemplate)
        {
            ...
            var month = this.invoiceInfo.DataInvoice.Value.Month;
            var year = this.invoiceInfo.DataInvoice.Value.Year;
            StringBuilder htmlFile = GetTemplateFileHtml(fullPathFileTemplate);
            htmlFile.Replace(ConfigInvoiceStatistical.CompanyName, this.invoiceInfo.CompanyName.ToUpper())
                               .Replace(ConfigInvoiceStatistical.CompanyAddress, this.invoiceInfo.Address.ToUpper())
                               .Replace(ConfigInvoiceStatistical.CompanyTaxCode, this.invoiceInfo.TaxCode)
                               .Replace(ConfigInvoiceStatistical.InvoiceNo, this.invoiceInfo.InvoiceNo)
                               .Replace(ConfigInvoiceStatistical.EnMonthYear, GetMonth(month) + " " + year.ToString())
                               .Replace(ConfigInvoiceStatistical.MonthYear, month.ToString() + "/" + year.ToString())
                               .Replace(ConfigInvoiceStatistical.InvoieDate, this.invoiceInfo.DataInvoice.Value.ToString("dd/MM/yyyy"))
                               .Replace(ConfigInvoiceStatistical.CustomerName, this.invoiceInfo.IsOrg ? this.invoiceInfo.CustomerCompanyName : this.invoiceInfo.CustomerName)
                               .Replace(ConfigInvoiceStatistical.CustomerTaxCode, this.invoiceInfo.CustomerTaxCode)
                               .Replace(ConfigInvoiceStatistical.CustomerAddress, this.invoiceInfo.CustomerAddress)
                               .Replace(ConfigInvoiceStatistical.Logo, logoBase64);
            return htmlFile;
        }
        ...
        private ExportFileInfo ExportInvoice(string fullPathFile, string fileName)
        {
            ...
            else
            {
                foreach (var item in DetailInvoice)
                {
                    ...
                    fileNameInvoice = string.Format("{0}\\{1}_{2}.pdf", fullPathFile, "invoice", DateTime.Now.ToString("yyyyMMddHHmmssfff"));
        ...
        private string ExportInvoiceJob()
        {
            StringBuilder htmlPdf = GetTemplateFileHtml(null);
            int identity = 1;
            int numberPage = DetailInvoice.Count;
        ...
        private void InvoiceHeader()
        {
            ...
            this.Header = new Dictionary<string, string>() {
                ...
                {InvoiceHeaderInfo.Address, this.invoiceInfo.Address},
                {InvoiceHeaderInfo.TaxCode, this.invoiceInfo.TaxCode},
                {InvoiceHeaderInfo.InvoiceCode, this.invoiceInfo.InvoiceCode},
                {InvoiceHeaderInfo.Symbol, this.invoiceInfo.Symbol},
                {InvoiceHeaderInfo.InvoiceNo, this.invoiceInfo.InvoiceNo},
                {InvoiceHeaderInfo.CustomerName, this.invoiceInfo.IsOrg ? this.invoiceInfo.CustomerCompanyName : this.invoiceInfo.CustomerName},
                ...
                {InvoiceHeaderInfo.ExchangeRate, String.Format(FormatNumber.GetStringFormat(this.SystemSetting?.ExchangeRate), this.invoiceInfo.ExchangeRate.ToDecimal())},
                {InvoiceHeaderInfo.CustomerAddress, this.invoiceInfo.CustomerAddress},
                {InvoiceHeaderInfo.TypePaymentCode,this.invoiceInfo.TypePayment},
                {InvoiceHeaderInfo.CustomerBankAccount, this.invoiceInfo.CustomerBankAccount},
                ...
                {InvoiceHeaderInfo.Mobile, this.invoiceInfo.Mobile},
                {InvoiceHeaderInfo.SpaceIfNotExistsTel, insertSpaceIfNoteixts(this.invoiceInfo.Tel)},
                {InvoiceHeaderInfo.Tel, !string.IsNullOrEmpty(this.invoiceInfo.Tel) ? this.invoiceInfo.Tel : spaces},
                ...
                {InvoiceHeaderInfo.BankAccount, this.invoiceInfo.BankAccount},
                {InvoiceHeaderInfo.BankName, this.invoiceInfo.BankName},
                {InvoiceHeaderInfo.Website, this.invoiceInfo.Website},
                {InvoiceHeaderInfo.TypePayment,this.invoiceInfo.TypePayment},
                {InvoiceHeaderInfo.CustomerCompanyName, this.invoiceInfo.CustomerCompanyName},
                {InvoiceHeaderInfo.ReportWebsite,this.invoiceInfo.ReportWebsite},
                {InvoiceHeaderInfo.ReportTel,this.invoiceInfo.ReportTel},
                {InvoiceHeaderInfo.EmailContract, this.invoiceInfo.EmailContract},
                {InvoiceHeaderInfo.CompanyUrl, $"{this.CurrentUser?.Company?.Id.ConvertToString()}"}
            };
            if (this.IsTemplateBIDC())
            {
                this.Header[InvoiceHeaderInfo.CustomerName] = this.invoiceInfo.CustomerName;
                this.Header[InvoiceHeaderInfo.ExchangeRate] = String.Format(FormatNumber.GetStringFormat(this.SystemSetting?.ExchangeRate), this.invoiceInfo.ExchangeRate.ToDecimal()) + " VND";
            }
            InvoiceHeaderChild();
            string logoSrc = string.Concat(this.config.FullPathFileAsset.Reverse().Skip(5).Reverse());   // Bỏ chữ Asset
            var LogoSrcConfig = this.GetConfig("LogoSrc").Replace('/', '\\');
            ...
            if (this.IsTemplateHuaNan() || this.IsTemplateMega())
            {
                this.Header.Add(InvoiceHeaderInfo.CompanyNameCn, this.GetConfig("CompanyNameCn"));
                this.Header.Add(InvoiceHeaderInfo.CompanyBranchCn, this.GetConfig("CompanyBranchCn"));
                this.Header.Add(InvoiceHeaderInfo.CompanyNameVn, this.GetConfig("CompanyNameVn"));
                this.Header.Add(InvoiceHeaderInfo.CompanyBranchVn, this.GetConfig("CompanyBranchVn"));
                this.Header.Add(InvoiceHeaderInfo.CompanyAddressRow1, this.GetConfig("CompanyAddressRow1"));
                this.Header.Add(InvoiceHeaderInfo.CompanyAddressRow2, this.GetConfig("CompanyAddressRow2"));
            }
        }
        private void InvoiceHeaderChild()
        {
            ...
            {
                this.Header.Add(InvoiceHeaderInfo.DayInvoice, this.invoiceInfo.DateRelease == null ? "..." : this.invoiceInfo.DateRelease?.Day.ToString().PadLeft(2, '0'));
                this.Header.Add(InvoiceHeaderInfo.MonthInvoice, this.invoiceInfo.DateRelease == null ? "..." : this.invoiceInfo.DateRelease?.Month.ToString().PadLeft(2, '0'));
                this.Header.Add(InvoiceHeaderInfo.YearInvoice, this.invoiceInfo.DateRelease == null ? "..." : this.invoiceInfo.DateRelease?.Year.ToString());
            }

            int invNo = 0;
            if (int.TryParse(this.invoiceInfo.InvoiceNo, out invNo))
            {
              ...
              string portalSearchInfo= string.Format("Hóa đơn điện tử được tra cứu trực tuyến tại: <a target=\"_blank\" href=\"{0}\">{1}</a> Mã tra cứu: {2}", UrlPortal, UrlPortalText, verificationCode);
                ...
                if (this.IsTemplateHuaNan() || this.IsTemplateMega() || this.IsTemplateBIDC())
                {
                    portalSearchInfo = string.Format("Hóa đơn điện tử được tra cứu trực tuyến tại: <a target=\"_blank\" href=\"{0}\">{1}</a> Mã tra cứu: {2}", UrlPortal, UrlPortalText, verificationCode);
                }
        ...
        {
            if (!invoiceInfo.isMultiTax || invoiceInfo.TemplateName == ExportInvoiceFileName.HuaNan_Detail)
            {
                if (invoiceInfo.Invoice_Status == (int)InvoiceStatus.Draft)
                {
                    htmlInvoice.Replace(InvoiceHeaderInfo.AmountTax, "");
                }
                else if (this.invoiceInfo.Tax.IsNullOrEmpty() || !this.invoiceInfo.TaxAmout.HasValue)
                {
                    htmlInvoice.Replace(InvoiceHeaderInfo.AmountTax, null);
                }
                else
                {
                    htmlInvoice.Replace(InvoiceHeaderInfo.AmountTax, String.Format(FormatNumber.GetStringFormat(this.SystemSetting?.Amount), this.invoiceInfo.TaxAmout.ToDecimal()));
                }
            }
            else
            {
                if (invoiceInfo.Invoice_Status == (int)InvoiceStatus.Draft)
                {
                    htmlInvoice.Replace(InvoiceHeaderInfo.AmountTax5, "");
                    htmlInvoice.Replace(InvoiceHeaderInfo.AmountTax10, "");
                }
                else if (this.invoiceInfo.Tax.IsNullOrEmpty() || !this.invoiceInfo.TaxAmout.HasValue)
                {
                    htmlInvoice.Replace(InvoiceHeaderInfo.AmountTax5, null);
                    htmlInvoice.Replace(InvoiceHeaderInfo.AmountTax10, null);
                }
                else
                {
                    var tax510 = CalculateTax510();
                    decimal tax5 = tax510.Item1;
                    decimal tax10 = tax510.Item2;
                    htmlInvoice.Replace(InvoiceHeaderInfo.AmountTax5, String.Format(FormatNumber.GetStringFormat(this.SystemSetting?.Amount), tax5.ToDecimal()));
                    htmlInvoice.Replace(InvoiceHeaderInfo.AmountTax10, String.Format(FormatNumber.GetStringFormat(this.SystemSetting?.Amount), tax10.ToDecimal()));
                }
            }
        }
        ...
        private void FillTotalInvoice(StringBuilder htmlInvoice, string page, bool isFillTotal = false)
        {
            ...
            if (invoiceInfo.InvoiceSample == (int)InvoiceType.Substitute || invoiceInfo.InvoiceSample == 0)
            {
                htmlInvoice.Replace(InvoiceHeaderInfo.displayDiscount, "<tr>");
                this.FooterInvoiceNotUpDown(htmlInvoice);
            }
            else if (invoiceInfo.InvoiceSample == (int)InvoiceType.AdjustmentUpDown || 
                invoiceInfo.InvoiceSample == (int)InvoiceType.AdjustmentTax || 
                invoiceInfo.InvoiceSample == (int)InvoiceType.AdjustmentTaxCode ||
                invoiceInfo.InvoiceSample == (int)InvoiceType.AdjustmentElse)
            {
                this.FillAmountUpDown(htmlInvoice);
            }
            else if (invoiceInfo.InvoiceSample == 3)
            {
            ...
            if (!invoiceInfo.DiscountAmount.HasValue)
            {
                htmlInvoice.Replace(InvoiceHeaderInfo.AmountDiscount, "");
            }
            else
            {
                htmlInvoice.Replace(InvoiceHeaderInfo.AmountDiscount, String.Format(FormatNumber.GetStringFormat(this.SystemSetting.Amount), this.invoiceInfo.DiscountAmount.ToDecimal()));
            }
            ...
            {
                htmlInvoice.Replace(InvoiceHeaderInfo.TotalDiscountTax, String.Format(FormatNumber.GetStringFormat(this.SystemSetting.Amount), this.invoiceInfo.TotalDiscountTax.ToDecimal()));
            }
            htmlInvoice.Replace(InvoiceHeaderInfo.AmountInwords, this.invoiceInfo.AmountInwords);
            var formatDecimalTotal = this.SystemSetting.Amount;
            ...
            htmlInvoice.Replace(InvoiceHeaderInfo.AmountTotal, String.Format(FormatNumber.GetStringFormat(this.SystemSetting.Amount), this.invoiceInfo.Sum.ToDecimal()));

            // Sửa chuyển phần thêm chữ ký mẫu về đây, dùng HTML chứ không dùng hình như trước để đồng nhất với template trên UI
            if (invoiceInfo.Invoice_Status > 3)
            {
                // Thiết lập CSS là visible
                string cssVisible = "visibility: visible;";
                htmlInvoice.Replace(InvoiceHeaderInfo.ShowSign, cssVisible);
                ...
                htmlInvoice.Replace(InvoiceHeaderInfo.SignDate, this.invoiceInfo.DataInvoice.Value.ToString("dd/MM/yyyy"));
            }
            else
            {
                string cssVisible = "visibility: hidden;";
                htmlInvoice.Replace(InvoiceHeaderInfo.ShowSign, cssVisible);
                //htmlInvoice.Replace(InvoiceHeaderInfo.SignDate, this.invoiceInfo.DataInvoice.ToString("dd/MM/yyyy"));
            }
        }
        ...
        {
            if (invoiceInfo.Invoice_Status == (int)InvoiceStatus.Draft
                || (IsTemplateBIDC() && this.invoiceInfo.Tax.IsNullOrEmpty()))
            {
                htmlInvoice.Replace(InvoiceHeaderInfo.TaxOfInvoice, string.Empty);
            }
            else if (this.invoiceInfo.Tax.IsNullOrEmpty())
            {
                htmlInvoice.Replace(InvoiceHeaderInfo.TaxOfInvoice, @"0%");
            }
            else
            {
                htmlInvoice.Replace(InvoiceHeaderInfo.TaxOfInvoice, this.invoiceInfo.TaxDisplay);
            }
        }
        ...
        private string SetDataToItemInvoice(int identity, InvoiceItem item, List<string> listProductName = null)
        {
            ...
            if (item.IsShowOrder)
            {
                if (this.IsTemplateShinhan())
                    orderRealNum = identity.ToString();
                else
                    orderRealNum = identity.ToString().PadLeft(2, '0');
            }

            if (item.ProductName.IsNullOrEmpty() && invoiceInfo.Invoice_Status != (int)InvoiceStatus.Draft) orderRealNum = "&nbsp;";
            ...
            htmlInvoice.Append(TemplateInvoiceDetail
                ...
                .Replace(ConfigInvoiceDetaiInfo.Quantity, formatDecimalBySystemSetting(item.Quantity, this.SystemSetting.Quantity))
                .Replace(ConfigInvoiceDetaiInfo.Discount, formatBoolBySystemSetting(item.Discount, this.SystemSetting.Amount))
                .Replace(ConfigInvoiceDetaiInfo.AmountDiscount, formatDecimalBySystemSetting(item.AmountDiscount, this.SystemSetting.Amount)));
            htmlInvoice.Replace(ConfigInvoiceDetaiInfo.Price, formatDecimalBySystemSetting(item.Price, this.SystemSetting.Price));
            htmlInvoice.Replace(ConfigInvoiceDetaiInfo.AmountTotal, formatNumberBySystemSetting(item.AmountTotal, this.SystemSetting.Amount));

            if (this.invoiceInfo.IsDiscount == 1)
            {
                htmlInvoice.Replace(ConfigInvoiceDetaiInfo.Tax, item.DiscountRatio.ToString());
                htmlInvoice.Replace(ConfigInvoiceDetaiInfo.AmountTax, formatDecimalBySystemSetting(item.AmountDiscount, this.SystemSetting.Amount));
            }
            else
            {
                htmlInvoice.Replace(ConfigInvoiceDetaiInfo.Tax, item.DisplayInvoice.IsNullOrEmpty() ? string.Empty : item.DisplayInvoice);
                htmlInvoice.Replace(ConfigInvoiceDetaiInfo.AmountTax, formatDecimalBySystemSetting(item.AmountTax, this.SystemSetting.Amount));
            }
                ...
                if ((invoiceInfo.Vat_invoice_type != null || !string.IsNullOrEmpty(invoiceInfo.Vat_invoice_type)) && identity == 1)
                {
                    if (invoiceInfo.Vat_invoice_type.Contains("3"))
                    {
                        ...
                        if (invoiceInfo.Report_class != null || !string.IsNullOrEmpty(invoiceInfo.Report_class))
                        {
                            ...
                            htmlInvoice.Replace(InvoiceHeaderInfo.AmountTotal, String.Format(FormatNumber.GetStringFormat(this.SystemSetting.Amount), this.invoiceInfo.Total.ToDecimal()));
                        }
                        else
                        {
        ...
        private void SubSetDataToItemInvoice(int identity, InvoiceItem item, StringBuilder htmlInvoice, List<string> listProductName = null)
        {
            ...
            htmlInvoice.Replace(ConfigInvoiceDetaiInfo.Total, item.Total == 0 ? string.Empty : item.Total.ToDecimalFomat(FormatNumber.GetStringFormat(this.SystemSetting.Amount)));
            htmlInvoice.Replace(ConfigInvoiceDetaiInfo.Price, item.Total == 0 ? string.Empty : item.Total.ToDecimalFomat(FormatNumber.GetStringFormat(this.SystemSetting.Price)));
            htmlInvoice.Replace(ConfigInvoiceDetaiInfo.Quantity, item.Quantity == 0 ? string.Empty : item.Quantity.ToDecimalFomat(FormatNumber.GetStringFormat(this.SystemSetting.Quantity)));
        ...
        private int CountRowForEachInvoiceItem()
        {
            ...
            if (this.IsTemplateHuaNan())
            {
                fieldLength = 2.43f;
            }
            if (this.IsTemplateMega())
            {
                fieldLength = 3.6f;
            }
            var i = 1;
            foreach (var item in this.invoiceInfo.InvoiceItems)
            {
                ...
                if (this.IsTemplateHuaNan() || this.IsTemplateMega() || this.IsTemplateBIDC() || this.IsTemplateSINO())
                {
                    numberRowInHtml = new NumberRowInHtml(item.ProductName, fieldLength, 12f);    // points = pixels * 72 / 96
                }
        ...
        private bool IsTemplateShinhan()
        {
            return ExportInvoiceFileName.IsTemplateShinhan(this.invoiceInfo.TemplateName);
        }

        private bool IsTemplateHuaNan()
        {
            return ExportInvoiceFileName.IsTemplateHuaNan(this.invoiceInfo.TemplateName);
        }

        private bool IsTemplateMega()
        {
            return ExportInvoiceFileName.IsTemplateMega(this.invoiceInfo.TemplateName);
        }

        private bool IsTemplateBIDC()
        {
            return ExportInvoiceFileName.IsTemplateBIDC(this.invoiceInfo.TemplateName);
        }
        private bool IsTemplateSINO()
        {
            return ExportInvoiceFileName.IsTemplateSINO(this.invoiceInfo.TemplateName);
        }
