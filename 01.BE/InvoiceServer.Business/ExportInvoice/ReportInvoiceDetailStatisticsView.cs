...
{
    public class ReportInvoiceDetailStatisticsView : IExportInvoiceBaseOnLang
    {
        ...
        public ReportInvoiceDetailStatisticsView(ReportCombineExport dataReports, SystemSettingInfo _systemSettingInfo, PrintConfig config, string language)
        {
            ...
            if (language == "en")
            {
                choooseFile = "invoice-detail-statistics-en.xlsx";
            }
            else
            {
                choooseFile = "invoice-detail-statistics-vn.xlsx";
            }
        ...
        {
            this.spireProcess.Worksheet.Copy(this.spireProcess.Worksheet.Range[string.Format("A{0}:O{0}", rowTemplate)], this.spireProcess.Worksheet.Range[string.Format("A{0}:O{0}", targetRow)], true);
        }
        ...
        private void SetDataToCells(ReportInvoiceDetail item, int order, int rowIndex, string lang)
        {
            ...
            this.spireProcess.Worksheet.Range[string.Format("I{0}", rowIndex)].Text = item.ProductName.ConvertToString();
            this.spireProcess.Worksheet.Range[string.Format("J{0}", rowIndex)].Text = item.Currency.ConvertToString();
            this.spireProcess.Worksheet.Range[string.Format("K{0}", rowIndex)].Value2 = item.CurrencyExchangeRate;
            this.spireProcess.Worksheet.Range[string.Format("K{0}", rowIndex)].NumberFormat = formatExchangeRate;
            this.spireProcess.Worksheet.Range[string.Format("L{0}", rowIndex)].Value2 = item.TotalAmount;
            this.spireProcess.Worksheet.Range[string.Format("L{0}", rowIndex)].NumberFormat = formatAmount;
            this.spireProcess.Worksheet.Range[string.Format("M{0}", rowIndex)].Text = item.TaxName.ConvertToString();
            this.spireProcess.Worksheet.Range[string.Format("N{0}", rowIndex)].Value2 = item.AmountTax;
            this.spireProcess.Worksheet.Range[string.Format("N{0}", rowIndex)].NumberFormat = formatAmount;
            this.spireProcess.Worksheet.Range[string.Format("O{0}", rowIndex)].Value2 = item.Total;
            this.spireProcess.Worksheet.Range[string.Format("O{0}", rowIndex)].NumberFormat = formatAmount;
            this.spireProcess.Worksheet.Range[string.Format("P{0}", rowIndex)].Text = InvoiceProperties.getInvoiceStatus((InvoiceStatus)item.InvoiceStatus, lang);
        ...
        private ExportFileInfo SaveFile(string fullPathFile)
        {
            ...
            string fileName = string.Format("{0}_{1}.xlsx", "invoice-detail-statistics", DateTime.Now.ToString(Formatter.DateTimeFormat));
        ...
        private void SetBorder(int rowIndex)
        {
            ...
            range.BorderAround();
            //range = this.spireProcess.Worksheet.Range[rowIndex, 17];
            //range.BorderAround();
        }
    }
}
